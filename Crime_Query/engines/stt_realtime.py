# has the input box different as st.text_input
...
def create_integrated_input_component():
    """
    Create an integrated text and voice input component.
    ...
    col_lang, col_main = st.columns([1, 4])
    ...
    with col_main:
        ...
        with col_input:
            ...
            current_text = st.text_input(
                "Enter your crime data query:",
                value=st.session_state.current_query,
                placeholder="e.g., Show me all assault crimes in Punjab",
                key="main_query_input",
                help="Type your query or use voice input"
            )

            # Update session state if text changed
            if current_text != st.session_state.current_query:
                st.session_state.current_query = current_text
        ...
        with col_voice:
            ...
            if st.button(
                    ...
                    key="voice_input_btn",
                    disabled=voice_button_disabled,  #False,
        ...
        with col_send:
            ...
            send_clicked = st.button(
                ...
                key="send_query_btn",
                help="Send query",
                disabled=not st.session_state.current_query.strip()
            )
    ...
    if st.session_state.show_voice_confirmation and st.session_state.pending_voice_text:
        ...
        with col_actions:
            ...
            if st.button("✅ Use This", key="confirm_voice"):
                ...
                # Add these lines for auto-submit functionality
    ...
    # Return query if send button was clicked
    if send_clicked and st.session_state.current_query.strip():
        query_to_return = st.session_state.current_query.strip()
...
def create_example_queries_section():
    ...
    examples = [
        ("Burglaries in Mumbai", "मुंबईत घरफोडीची प्रकरणे दर्शवा"),
        ("Thefts in Bihar", "बिहार में चोरी देखावा"),
        ("Theft case reported by Sanjay Verma in Tamil Nadu", "தமிழ்நாட்டில் சஞ்சய் வர்மா அவரால் புகார் செய்யப்பட்ட திருட்டு வழக்கு"),
        ("Assaults in Punjab", "ਪੰਜਾਬ ਵਿੱਚ ਹਮਲਿਆਂ ਨੂੰ ਵਿਖਾਓ"),
        ("Cybercrime in Bengalore", "Show me Cybercrimes in Bangalore"),
        ("Vandalism in Kolkata damaging public property", "কলকাতা ভাঙচুরের ঘটনা দেখাও, যেখানে সাধারণ সম্পত্তির ক্ষতি হয়েছে, যেমন: রাস্তার ল্যাম্পপোস্ট ভেঙে ফেলা হয়েছে।")
    ]
